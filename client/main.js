 ...
 * Isso evita que nomes de jogadores sejam interpretados como HTML.
 */
 ...
 */
const getName = () =>
  localStorage.getItem("blast-name") || "Jogador";

/**
 * Converte milissegundos para o formato minutos:segundos.
 */
...
/**
 * Retorna o HTML da logo do Bomberlan.
 */
function brand() {
  return `
    <a class="brand" href="/" aria-label="Página inicial do Bomberlan">
      <img
...
function renderLanding() {
    ...
    <main class="menu-home ${showIntro ? "has-intro" : ""}">
          ...
          ? `
            <div class="menu-intro" id="menu-intro">
              ...
              <div class="intro-loader">
                ...
                <span>
                  <b>PREPARANDO A ARENA</b>
                  <em>CARREGANDO SISTEMAS</em>
                </span>
              </div>
              ...
              <section
                ...
                aria-live="polite"
              >
                <span class="intro-story-kicker">
                  LUASOFT APRESENTA
                </span>

                <h1>
                  BEM-VINDO AO
                  <strong>BOMBERLAN</strong>
                </h1>

                <p>
                  Uma arena multiplayer em pixel art onde
                  estratégia, bombas e coragem decidem o
                  último sobrevivente.
                </p>
      ...
      <section class="menu-stage">
        ...
        <div class="menu-options">
          ...
          <form
            class="arcade-menu-form"
            id="create-form"
          >

            <button
              class="arcade-action create-room-action"
              type="submit"
            >
              <span>
        ...
        >
          ...
          <canvas
            ...
            width="360"
            height="432"
            aria-label="Personagem do Bomberlan fazendo malabarismo com bombas"
          ></canvas>
  ...
  // Introdução mostrada apenas na primeira entrada.
  if (showIntro) {
    ...
    startButton?.addEventListener(
      ...
      () => {
        ...
        setTimeout(() => {
          intro?.classList.add("story-mode");
        }, 1400);
    ...
    .addEventListener("submit", async (event) => {
      ...
      try {
        ...
        root.querySelector(
          ...
          "A arena está aquecendo. Tente novamente.";
...
function saveName() {
  const name =
    (
      root.querySelector("#player-name")?.value ||
      getName()
    )
      .trim()
      .slice(0, 16) || "Jogador";
  ...
  root.innerHTML = `
    <main class="center-shell">
      ...
      >
        ...
        <p>
          Escolha seu nome e entre na arena.
        </p>
    ...
    <main class="room-transition">
      ...
      >
        ...
        <div class="transition-copy">
          ...
          <span>
            Sincronizando jogadores e preparando o campo
          </span>
        </div>
  ...
  socket.addEventListener(
    ...
    () => {
      ...
      if (!errorShown && player) {
        showToast(
          "Conexão perdida. Atualize a página para voltar.",
          true,
        );
      }
    },
...
function transitionToMatch(start) {
  ...
  root.insertAdjacentHTML(
    ...
    `
      <section
        class="match-loading-transition"
        role="status"
        aria-live="assertive"
      >

        <div
          class="match-loading-grid"
          aria-hidden="true"
        ></div>

        <div
          class="match-loading-beam"
          aria-hidden="true"
        ></div>
...
function renderMatch(start) {
  ...
  // Zera os controles antes de começar uma nova partida.
  localInput = {
    dx: 0,
    dy: 0,
    drop: false,
  ...
  const roster =
    start.players
      .map(
        (candidate) => `
          <span
            style="--intro-color:${PLAYER_COLORS[candidate.slot]}"
          >
  ...
  root.innerHTML = `
    <main class="game-shell match-pending">
      ...
      <header class="game-header">
        ...
        <div class="match-label">
          ...
          <span>
            SALA ${escapeHtml(player.roomCode)}
          </span>
      ...
      <section class="game-layout">
        ...
        <aside class="match-sidebar">
          ...
          <div class="panel-title">
            ...
            <!--
              Usa a quantidade real de participantes.
              Isso é importante para partidas com mais de 4 jogadores.
            -->
            <b id="alive-count">
              ${start.players.length} VIVOS
            </b>
      ...
      <div
        ...
        aria-label="Controles de toque do jogo"
      >
    ...
    >
      ...
      <div
        class="match-intro-grid"
        aria-hidden="true"
      ></div>

      <div
        class="match-intro-burst"
        aria-hidden="true"
      ></div>

      <div class="match-intro-content">

        <span class="match-intro-kicker">
          SALA ${escapeHtml(player.roomCode)}
          · MODO CLÁSSICO
        </span>

        <div class="match-intro-logo">
          <i></i>
        ...
        <div class="countdown-stage">
          ...
          <em id="countdown-label">
            PREPARE-SE
          </em>

        </div>

        <div class="match-intro-roster">
          ${roster}
        </div>
  ...
  root.querySelector(
    ...
    isSuperMode
      ? "SUPER BOMBERLAN · POWER-UPS ATIVOS"
      : "BOMBERLAN · COMBATE PURO";

  root.querySelector(
    ".match-intro-kicker",
  ).textContent =
    `SALA ${player.roomCode} · ${
      isSuperMode
        ? "SUPER BOMBERLAN"
        : "BOMBERLAN"
    }`;

  // Controles extras do modo Super.
  if (isSuperMode) {
    root
      .querySelector(
        ".controls-card",
      ...
      ?.insertAdjacentHTML(
        ...
        `
          <p>
            <kbd>E</kbd>
            Detonar controle remoto
          </p>

          <p>
            <kbd>Q</kbd>
            Arremessar com a luva
          </p>
        `,
      );
      ...
      ?.insertAdjacentHTML(
        ...
        `
          <div class="power-buttons">
            ...
            <button
              data-action="detonate"
              aria-label="Detonar bomba remota"
            >
              REMOTO
            </button>
            ...
            <button
              data-action="special"
              aria-label="Arremessar bomba com a luva"
            >
              LUVA
            </button>
  ...
  // Primeira renderização da arena.
  renderGame(
  ...
  /**
   * Atualiza o número do countdown
   * reiniciando a animação visual.
   */
...
function renderResult(
  ...
  // Relaciona jogador e quantidade de troféus.
  const scoreById =
    new Map(
      standings.map(
        (entry) => [
  ...
  // Cria a classificação da rodada.
  const ranking =
    competitors
      .map(
        (candidate) => ({
    ...
    root.insertAdjacentHTML(
      ...
      `
        ...
        >
          ...
          <div
            class="winner-celebration-burst"
            aria-hidden="true"
          ></div>
          ...
          <canvas
            width="112"
            height="96"
            aria-label="${escapeHtml(winner.name)} comemorando a vitória"
          ></canvas>
          ...
          <p>
            O campeão está pronto para receber o troféu.
          </p>
  ...
  const mountResultBoard = () => {
          ...
          (
            ...
            const character =
              isDraw
                ? `
                  <img
                    class="battle-avatar draw-result-avatar"
                    src="/player-avatar-${candidate.slot + 1}.png"
                    alt=""
                  />
                `
                : `
                  <canvas
                    class="battle-avatar result-character-canvas"
                    width="112"
                    height="96"
                    data-result-slot="${candidate.slot}"
                    data-result-mood="${mood}"
                    role="img"
                    aria-label="${escapeHtml(candidate.name)} ${
                      isWinner
                        ? "comemorando"
                        : "chorando"
                    }"
            ...
            return `
              <article
                ...
                "
                style="
                  --rank-color:${PLAYER_COLORS[candidate.slot]};
                  --rank-delay:${0.24 + index * 0.12}s
                "
              >

                <span class="battle-position">
                  ${
                    isDraw
                      ? "="
                      : `${index + 1}º`
    ...
    root.insertAdjacentHTML(
      "beforeend",
      `
        <section class="result-overlay battle-result-overlay">

          <div
            class="result-pixel-rain"
            aria-hidden="true"
          >
            <i></i>
            <i></i>
          ...
          <div
            ...
            "
            style="
              --trophy-delay:${TROPHY_ANIMATION_DELAY_MS}ms;
              --trophy-duration:${TROPHY_ANIMATION_DURATION_MS}ms
            "
          >

            <header class="battle-result-header">
            ...
            <div
              class="battle-ranking"
              aria-label="Classificação da sala"
            >
              ${rankingMarkup}
            </div>
    ...
    // Inicia animações dos personagens do placar.
    const characterStops = [
      ...root.querySelectorAll(
        ".result-character-canvas",
      ),
    ].map((canvas) =>
      startResultCharacterAnimation(
        canvas,
        Number(
          canvas.dataset.resultSlot,
        ),
        canvas.dataset.resultMood,
      ),
    );
...
function renderError(message) {
  ...
  root.innerHTML = `
    <main class="center-shell">
      ...
      <section class="compact-card error-card">

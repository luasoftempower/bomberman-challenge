...
test("the seventh human receives ROOM_FULL", () => {
  const room = new Room(
    "ABC234",
    "host-secret",
  );

  // Os 6 primeiros jogadores devem conseguir entrar.
  for (
    let count = 0;
    count < 6;
    count += 1
  ) {
    const joined = room.addHuman(
      new FakeSocket(),
      {
        name: `P${count}`,
        hostToken:
...
test("stale input packets cannot overwrite a newer direction", () => {
  ...
  assert.equal(
    room.inputs[host.id].dx,
    0,
  );

  assert.equal(
    room.inputs[host.id].dy,
    1,
  );

  assert.deepEqual(
    room.inputs[host.id].intent,
    {
      dx: 0,
      dy: 1,
    },
...
test("starting with two humans creates exactly two bots", () => {
  ...
  // Com até 4 participantes a arena continua quadrada.
  assert.equal(
    room.state.arenaType,
    "square",
  );
...
test("only the lone host can choose the bot difficulty", () => {
  ...
  assert.equal(
    hostSocket.messages.at(-1)
      .botDifficulty,
    "hard",
  );

  const guest = room.addHuman(
    new FakeSocket(),
    {
      name: "Friend",
    },
...
test("the simulation waits for the animated match countdown", () => {
  ...
  assert.equal(
    matchStart.countdownMs,
    4420,
  );

  assert.equal(
    matchStart.durationMs,
    90_000,
  );
